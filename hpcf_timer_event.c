 ...
 *     right 是右孩子。
 */
static Node* create_rbtree_node(Type key, struct timer_event *t_event, Node *parent, Node *left, Node* right)
{
    Node* p;
    ...
    p->t_event.interval = t_event->interval;
    p->t_event.count = t_event->count;
    p->t_event.persisent = t_event->persisent;
    p->t_event.data = &p->t_event;
    p->t_event.function = t_event->function;
    p->t_event.event_callback = t_event->event_callback;
    p->t_event.root = t_event->root;

    p->key = key + p->t_event.interval;
    p->t_event.key = p->key;
...
int insert_rbtree(RBRoot *root, Type key, struct timer_event *t_event)
{
    ...
    // 如果新建结点失败，则返回。
    if ((node=create_rbtree_node(key, t_event, NULL, NULL, NULL)) == NULL)
        return -1;
...
Type hpcf_get_current_time()
{
    ...
    current_msec = t.tv_sec * 1000;
    current_msec += (t.tv_usec / 1000);

    return current_msec;
}

// 创建下一个定时任务
void hpcf_create_next_timer_event(void *arg)
{
    struct timer_event *t_event = (struct timer_event *)arg;
    struct timer_event new_event;

    // 创建新的定时任务
    // 直接添加到红黑树中就可以了
    new_event.persisent = t_event->persisent;

    // 如果是持久性定时任务，需要创建下一个定时任务
    if (!new_event.persisent) {
        return ;
    }
    ...
    // 添加新的定时任务
    insert_rbtree(new_event.root, current_time, &new_event);

    // 删除旧的定时任务
    delete_rbtree(t_event->root, t_event->key);
}
...
int hpcf_find_sleep_time(RBRoot *root)
{
    ...
    // 如果红黑树不为空，返回最小的时间
    Node *min_node = minimum(node);
    int sleep_time = min_node->key - current_time;
...
void hpcf_execute_timer_event(RBRoot *root)
{
    ...
    // 执行定时任务
    do {
        typedef void (*timer_event_handler)(void *arg);
        timer_event_handler handler = (timer_event_handler)min_node->t_event.event_callback;
        handler(NULL);

        // 如果是持久性定时任务，需要创建下一个定时任务
        if (min_node->t_event.persisent) {
            hpcf_create_next_timer_event(&min_node->t_event);
        } else {
            // 删除旧的定时任务
            delete_rbtree(root, min_node->key);
        }

        // 找到下一个定时任务
        min_node = minimum(node);

    } while(min_node->key < current_time);
